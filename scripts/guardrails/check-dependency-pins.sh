#!/bin/bash
# Guardrail: Validate dependency files have pinned versions before writes are accepted.
# Expected input: JSON payload on stdin from a wrapper or CI harness.
...
# Single Python invocation: extract all ops and validate each dependency file.
