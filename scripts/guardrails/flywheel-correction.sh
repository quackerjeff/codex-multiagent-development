#!/bin/bash
# Helper: Detect correction signals in user prompts and log them for flywheel analysis.
# Example wrapper event: user prompt submit
# Writes filtered JSONL entries to ~/.codex/flywheel-corrections.jsonl
#
...
#
# The helper outputs nothing on stdout (we don't want to inject anything into
...
LOG_PATH = os.path.realpath(os.path.expanduser("~/.codex/flywheel-corrections.jsonl"))
...
try:
    ...
    print(f"Flywheel helper error: failed to parse event: {e}", file=sys.stderr)
    sys.exit(1)
...
# Some wrappers pass only the user prompt; others may include
